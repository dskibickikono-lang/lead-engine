...
- **Database Concurrency:** The reliance on a single SQLite DB with `_pragma=busy_timeout(5000)` might become a bottleneck under significant concurrent write load if scrapers or enrichment scale up.
...
- **Resilient Runner:** The `internal/run/runner.go` is well-designed. It records stage outcomes in SQLite (`run_stages`) and supports resuming from the last failed stage. This is a critical operational feature for a daily cron job.
...
- **Clear Schema Definitions:** The raw-leads contract (`internal/contract/contract.go`) is strict and type-safe, validating required fields like `ExternalID` and `CompanyName`.
...
- **Alternative:** Expose scrapers as internal HTTP services or CLI tools that output JSON to stdout. The orchestrator can execute them and consume the stdout stream directly (`json.NewDecoder`), avoiding intermediate files and reducing state.
...
- **Alternative:** Use SQLite in WAL mode with a higher `MaxOpenConns` (e.g., 4-8) and handle `database is locked` retries at the application layer, or use a connection pool dedicated to writes vs. reads.
...
#### Go Orchestrator
- **Ingestion:** `ingest.Ingest` is clean, but it marshals the entire `Offer` into a JSON string (`Payload`) in the DB. This duplicates data already stored in columns.
- **Enrichment:** `internal/enrich/enrich.go` ignores context cancellation during the main loop iterations. It only checks `ctx.Err()` at the start of the loop. If a context is cancelled, the current slow HTTP request won't be interrupted.
...
#### General Risks
- **Missing Tests:** End-to-end integration tests are missing. There should be a test that runs the orchestrator with mocked scrapers and APIs.
...
1. **Parallelize Enrichment (M Effort, High Impact)**
...
2. **Pydantic Export Contract Validation (S Effort, High Impact)**
   - **What:** Implement Pydantic models in `gov_api` and `olx-printing-press` for the output contract.
   - **Why:** Prevents runtime ingestion failures in the orchestrator by guaranteeing schema compliance at the source.
   - **Implementation:** Replace the manual dictionary construction in `exporter.py` and Go JSON structs with formal models.

3. **Improve Name Matching Heuristics (L Effort, Medium Impact)**
...
4. **Systemd Daemonization (S Effort, Medium Impact)**
   - **What:** Replace `cron` with a `systemd` service and timer.
   - **Why:** Improves operational visibility, log management (via `journalctl`), and automatic restarts on failure.
